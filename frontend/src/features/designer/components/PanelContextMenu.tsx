...
import { MIN_DOOR_WIDTH, MIN_WALL_WIDTH } from '../lib/panelActions'
...
function Section({ label, children, dimmed = false }: { label: string; children: React.ReactNode; dimmed?: boolean }) {
  return (
    <div className={cn('flex flex-col gap-2', dimmed && 'opacity-35 pointer-events-none select-none')}>
      <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400 dark:text-white/35">
        {label}
      </span>
      {children}
    </div>
...
function Stepper({
  ...
  return (
    <div className="flex items-center gap-0 overflow-hidden rounded-xl border border-black/10 bg-black/[0.04] dark:border-white/10 dark:bg-white/[0.04]">
      <button
        type="button"
        disabled={atMin}
        ...
        className="flex h-11 w-11 shrink-0 items-center justify-center text-lg font-light
                   text-gray-500 transition hover:bg-black/8 hover:text-gray-900
                   disabled:opacity-25 active:bg-black/12
                   dark:text-white/50 dark:hover:bg-white/8 dark:hover:text-white dark:active:bg-white/12"
      >
        −
      </button>

      <div className="flex flex-1 items-center justify-center gap-1.5 border-x border-black/10 py-2 dark:border-white/10">
        <input
          type="number"
          value={value}
          onChange={(e) => {
          ...
          className="w-20 bg-transparent text-center font-mono text-base font-semibold
                     text-gray-900 outline-none dark:text-white [appearance:textfield]
                     [&::-webkit-inner-spin-button]:appearance-none
                     [&::-webkit-outer-spin-button]:appearance-none"
        />
        <span className="text-xs font-medium text-gray-400 dark:text-white/30">мм</span>
      </div>
...
function ToggleGroup<T extends string>({
  ...
  return (
    <div className="flex gap-1.5 rounded-xl border border-black/10 bg-black/[0.04] p-1 dark:border-white/10 dark:bg-white/[0.04]">
      {options.map((opt) => (
        <button
          key={opt.value}
          type="button"
          ...
          className={cn(
            ...
            value === opt.value
              ...
              : 'text-gray-500 hover:bg-black/8 hover:text-gray-900 dark:text-white/45 dark:hover:bg-white/8 dark:hover:text-white',
          )}
        >
...
function GhostBtn({ icon, label, onClick }: { icon: string; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-xl border border-black/8
                 bg-black/[0.03] px-4 py-3 text-sm text-gray-500
                 transition hover:border-black/15 hover:bg-black/8 hover:text-gray-900
                 active:scale-[0.98]
                 dark:border-white/8 dark:bg-white/[0.03] dark:text-white/60
                 dark:hover:border-white/15 dark:hover:bg-white/8 dark:hover:text-white"
    >
    ...
    <div className="flex flex-col gap-5 p-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-lg
                     text-xs text-gray-400 transition hover:bg-black/8 hover:text-gray-600
                     dark:text-white/25 dark:hover:bg-white/8 dark:hover:text-white/60"
        >
          ✕
        </button>
      </div>
      ...
      <Section label={panel.isDoor ? 'Ширина двери' : 'Ширина панели'}>
        <Stepper
          ...
          step={50}
          min={panel.isDoor ? MIN_DOOR_WIDTH : MIN_WALL_WIDTH}
          max={panel.isDoor ? maxDoor : 3000}
        />
        <p className="text-[11px] text-gray-400 dark:text-white/25">
          {'Мин. '}
          {panel.isDoor ? MIN_DOOR_WIDTH : MIN_WALL_WIDTH}
          {' мм'}
          {panel.isDoor && ` · макс. ${maxDoor} мм`}
        </p>
      </Section>
      ...
      <>
        ...
        <div
          ...
          className="fixed inset-x-0 bottom-0 z-50 max-h-[90vh] overflow-y-auto
                     rounded-t-2xl border-t border-black/10 bg-white
  ...
  return (
    <div
      ...
      className="overflow-hidden rounded-2xl border border-black/10 bg-white
                 ...
                 dark:border-white/10 dark:bg-[#0d1117]/95 dark:shadow-black/70"
    >
      {content}
    </div>
  )
